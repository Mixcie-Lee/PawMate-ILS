...
private fun buildShopItems(context: android.content.Context): List<MerchandiseItem> {
    ...
    if (hiRefs.isNotEmpty()) {
        val names = listOf(
            "PawMate Graphic Tee",
            "PawMate Collectible Figure",
            "PawMate Insulated Mug",
            "PawMate Hoodie",
            "PawMate Keychain",
            "PawMate Tote Bag",
            "PawMate Sticker Pack",
            "PawMate Desk Figurine"
        )
...
fun ShopScreen(navController: NavController) {
    ...
    val cartItems = remember { mutableStateListOf<CartEntry>() }
    val boughtItems = remember { mutableStateListOf<CartEntry>() }
    ...
    fun applyPromoCode() {
        ...
        when {
            normalized.isEmpty() -> {
                appliedPromoCode = ""
                promoStatusMessage = "Enter a promo code first."
            }
            normalized == "PROMO1" -> {
                appliedPromoCode = normalized
                promoStatusMessage = "Promo applied: 20% off."
            }
            else -> {
                appliedPromoCode = ""
                promoStatusMessage = "Invalid promo code."
            }
        }
    }
    ...
    Box(
        ...
        Scaffold(
            ...
            LazyColumn(
                    ...
                    Card(
                        ...
                        Box(
                            modifier = Modifier
                                ...
                                .background(
                                    Brush.horizontalGradient(
                                        listOf(Color(0xFFDCCBFF), Color(0xFFD8F2FF), Color(0xFFF7DDFD))
                                    ),
                            ...
                            Column(
                                ...
                                Text(
                                    text = "50% OFF",
                                    fontSize = if (isTablet) 46.sp else 42.sp,
                                    fontWeight = FontWeight.ExtraBold,
                                    color = Color(0xFF9A8AD8)
                                )
                                Text(
                                    text = "Limited PawMate merch deals",
                                    fontSize = if (isTablet) 14.sp else 12.sp,
                                    color = Color(0xFF7D77CF)
                                )
                            }
                            Text(
                                text = "💎 GEMS",
                    ...
                    LazyRow(horizontalArrangement = Arrangement.spacedBy(6.dp)) {
                        items(categories) { category ->
                            ...
                            AssistChip(
                                ...
                                label = { Text(if (category == "All") "New" else category) },
                                colors = AssistChipDefaults.assistChipColors(
                                    containerColor = if (selected) Color(0xFFFFCDD9) else Color(0xFFF7E6EA),
                                    labelColor = Color(0xFFC56D7D)
                                )
                            )
                        }
                    }
                        ...
                        Row(
                            ...
                            OutlinedTextField(
                                ...
                                leadingIcon = {
                                    Text(
                                        "P",
                                        color = Color(0xFFC16565),
                                        fontWeight = FontWeight.Bold
                                    )
                                },
                                placeholder = { Text("Promo code (use PROMO1)") },
                            ...
                            Button(
                                ...
                                colors = ButtonDefaults.buttonColors(
                                    containerColor = if (promoApplied) Color(0xFF4CAF50) else Color(0xFFC16565)
                                ),
                                shape = RoundedCornerShape(10.dp)
                            ) {
                                Text(
                                    text = if (promoApplied) "APPLIED" else "ENTER",
                                    color = Color.White,
                                    fontWeight = FontWeight.Bold
                                )
                            }
                        }
                        promoStatusMessage?.let { msg ->
                            Text(
                                text = msg,
                                color = if (promoApplied) Color(0xFF4CAF50) else Color(0xFFC16565),
                                fontSize = 12.sp,
                ...
                val rowChunkSize = if (isTablet) 3 else 2
                items(filtered.chunked(rowChunkSize)) { rowItems ->
                    Row(horizontalArrangement = Arrangement.spacedBy(8.dp), modifier = Modifier.fillMaxWidth()) {
                        rowItems.forEach { product ->
                            Card(
                                ...
                                colors = CardDefaults.cardColors(containerColor = Color(0xFFFFC9BF)),
        ...
        Box(
            ...
            Card(
                shape = CircleShape,
                colors = CardDefaults.cardColors(containerColor = Color(0xFFE68F8F)),
            ...
            AlertDialog(
                ...
                title = {
                    Row(
                        ...
                        Icon(Icons.Default.ShoppingCart, contentDescription = null, tint = Color(0xFFC16565))
                        Text("My Cart", fontWeight = FontWeight.Bold)
                        if (cartItems.isNotEmpty()) {
                            Card(
                                colors = CardDefaults.cardColors(containerColor = Color(0xFFC16565)),
                    ...
                    Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                        if (cartItems.isEmpty() && boughtItems.isEmpty()) {
                            Column(
                                ...
                                Text("Your cart is empty", fontWeight = FontWeight.Medium, color = Color.Gray)
                                Text("Add items from the product list.", fontSize = 12.sp, color = Color.Gray.copy(alpha = 0.7f))
                            }
                        }
                        if (cartItems.isNotEmpty()) {
                            cartItems.forEach { entry ->
                                val unit = parsePrice(entry.item.price)
                                val discountedUnit = if (promoApplied) unit * 0.8 else unit
                                val subtotal = discountedUnit * entry.quantity
                                Card(
                                    modifier = Modifier.fillMaxWidth(),
                                    ...
                                    Row(
                                        ...
                                        Column(modifier = Modifier.weight(1f)) {
                                            Text(entry.item.name, fontWeight = FontWeight.SemiBold, maxLines = 1, fontSize = 14.sp)
                                            Text(entry.item.category, color = Color.Gray, fontSize = 11.sp)
                                        }
                                        Column(horizontalAlignment = Alignment.End) {
                                            Text("x${entry.quantity}", fontWeight = FontWeight.Bold, fontSize = 13.sp)
                                            Text(
                                                formatPhp(subtotal),
                                                color = Color(0xFFC16565),
                                                fontWeight = FontWeight.Bold,
                            ...
                            val cartTotal = cartItems.sumOf { entry ->
                                val unit = parsePrice(entry.item.price)
                                (if (promoApplied) unit * 0.8 else unit) * entry.quantity
                            }
                            Row(
                                modifier = Modifier.fillMaxWidth(),
                                ...
                                Text(formatPhp(cartTotal), fontWeight = FontWeight.ExtraBold, color = Color(0xFFC16565))
                            }
                            if (promoApplied) {
                                Text("PROMO1 applied: 20% off", color = Color(0xFF4CAF50), fontSize = 12.sp)
                            }
                        }
                        if (boughtItems.isNotEmpty()) {
                            HorizontalDivider(modifier = Modifier.padding(vertical = 4.dp))
                            Text("Recent purchases", fontWeight = FontWeight.SemiBold, fontSize = 13.sp, color = Color.Gray)
                            boughtItems.takeLast(5).forEach { entry ->
                                Card(
                                    modifier = Modifier.fillMaxWidth(),
                                    ...
                                    Row(
                                        ...
                                        Column(modifier = Modifier.weight(1f)) {
                                            Text(entry.item.name, fontWeight = FontWeight.Medium, maxLines = 1, fontSize = 13.sp)
                                            Text("x${entry.quantity}", color = Color.Gray, fontSize = 11.sp)
                                        }
                                    }
                                }
                            }
                ...
                confirmButton = {
                    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        ...
                        if (cartItems.isNotEmpty()) {
                            Button(
                                onClick = {
                                    cartItems.forEach { addOrMergeEntry(boughtItems, it.item, it.quantity) }
                                    cartItems.clear()
            ...
            AlertDialog(
                ...
                text = {
                    Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                        ...
                        Row(
                            ...
                            Column {
                                Row(horizontalArrangement = Arrangement.spacedBy(6.dp)) {
                                    Card(
                                        colors = CardDefaults.cardColors(containerColor = Color(0xFFFFCDD9)),
                                        shape = RoundedCornerShape(8.dp)
                                    ) {
                                        Text(
                                            product.category,
                        ...
                        OutlinedTextField(
                            ...
                            modifier = Modifier.fillMaxWidth(),
                            singleLine = true,
                            leadingIcon = { Text("P", color = Color(0xFFC16565), fontWeight = FontWeight.Bold) },
                            placeholder = { Text("Enter promo code") },
                            shape = RoundedCornerShape(12.dp),
                            colors = OutlinedTextFieldDefaults.colors(
                                focusedContainerColor = Color(0xFFFFF5F8),
                        ...
                        Text(
                            if (promoApplied) "Promo applied: 20% discount enabled."
                            else "Use PROMO1 for 20% off.",
                            fontSize = 12.sp,
                            color = if (promoApplied) Color(0xFF4CAF50) else Color.Gray
                        )
                        Row(
                            verticalAlignment = Alignment.CenterVertically,
                            horizontalArrangement = Arrangement.spacedBy(10.dp)
                        ) {
                            Text("Qty", fontWeight = FontWeight.Medium)
                            Button(
                                onClick = { if (quantity > 1) quantity -= 1 },
                                colors = ButtonDefaults.buttonColors(containerColor = Color(0xFFFFCDD9))
                            ) { Text("-", color = Color(0xFF9A4E5F)) }
                        ...
                        Text(
                            "Choose your quantity then add to cart or buy now.",
                            fontSize = 12.sp,
                            color = Color.Gray
                        )
                    }
                ...
                dismissButton = {
                    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        ...
                        Button(
                            onClick = {
                                addOrMergeEntry(boughtItems, product, quantity)
                                selectedProduct = null
                                showCartDialog = true
                            },
                            colors = ButtonDefaults.buttonColors(containerColor = Color(0xFFC16565))
